...
#include "../../../../../../../../../Library/Android/sdk/ndk-bundle/platforms/android-15/arch-mips/usr/include/stdio.h"
...
void mono_wave_read(MONO_PCM *pcm, char *file_name)
{
    ...
    pcm->s = (jdouble*)calloc(pcm->length, sizeof(jdouble)); /* メモリの確保 */

    for (n = 0; n < pcm->length; n++)
    {
        fread(&data, 2, 1, fp); /* 音データの読み取り */
        pcm->s[n] = (jdouble)data / 32768.0; /* 音データを-1以上1未満の範囲に正規化する */
        //pcm->s[n] = (double)data; /* 音データを-1以上1未満の範囲に正規化する */
    }
...
void mono_wave_read2(MONO_PCM2 *pcm, char *file_name)
{
    FILE *fp;
    jint n;
    jchar riff_chunk_ID[4];
    jlong riff_chunk_size;
    jchar riff_form_type[4];
    jchar fmt_chunk_ID[4];
    jlong fmt_chunk_size;
    jshort fmt_wave_format_type;
    jshort fmt_channel;
    jlong fmt_samples_per_sec;
    jlong fmt_bytes_per_sec;
    jshort fmt_block_size;
    jshort fmt_bits_per_sample;
    jchar data_chunk_ID[4];
    jlong data_chunk_size;
    jshort data;
...
double *mono_wave_read3(char *file_name)
{
    FILE *fp;
    int n;
    int dataSize = 300000;
...
//    pcm->fs = fmt_samples_per_sec; /* 標本化周波数 */
  //  pcm->bits = fmt_bits_per_sample; /* 量子化精度 */
  //  pcm->length = data_chunk_size / 2; /* 音データの長さ */
   // pcm_d = (double*)calloc(2048, sizeof(double)); /* メモリの確保 */

     pcm_d = (double*)calloc(dataSize, sizeof(double)); /* メモリの確保 */
...
//    for (n = 0; n < 2048; n++)
        ...
        {
        fread(&data, 2, 1, fp); /* 音データの読み取り*/
      //  pcm_d[n] = (double)data / 32768.0; /* 音データを-1以上1未満の範囲に正規化する */
        pcm_d[n] = (double)data / 32768.0; /* 音データを-1以上1未満の範囲に正規化する */
...
long mono_wave_read4(char *file_name)
{
    FILE *fp;
    int n;
    int dataSize = 15000;
...
//    for (n = 0; n < 2048; n++)
    ...
    return fmt_chunk_size;
}


char* mono_wave_read5(char *file_name)
{
    FILE *fp;
    int n;
    int dataSize = 15000;
